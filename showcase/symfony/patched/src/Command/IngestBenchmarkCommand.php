...
#[AsCommand(
    ...
    description: 'Benchmark API data ingestion and processing (patched PHP with typed arrays)',
)]
class IngestBenchmarkCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('API Ingestion Benchmark - PATCHED PHP');
        $io->text([
        ...
        $io->section('Benchmark 2: Normalizing user data (with typed shapes)...');
        $start = hrtime(true);
        $normalizedCount = 0;
        ...
        $io->section('Benchmark 3: Transforming to typed API response format...');
        $start = hrtime(true);
        $transformedCount = 0;
        ...
        $io->section('Benchmark 4: Aggregating statistics (with typed return)...');
        $start = hrtime(true);
        ...
        $io->text([
            ...
            '  Countries: ' . count($stats['by_country']),
            '  Genders: ' . implode(', ', array_keys($stats['by_gender'])),
        ]);
        ...
        $io->section('Benchmark 6: Building typed paginated responses...');
        $start = hrtime(true);
        $pageCount = 0;
     ...
     * Returns a NormalizedUser - runtime validated.
     */
     ...
     * Returns a UserResponse - runtime validated.
     */
     ...
     * Returns a UserStats - runtime validated.
     */
